...
type License = {
  id: string; license_prefix: string; customer_note: string | null; status: string;
...
export default function AdminClient() {
  ...
  async function createLicense(e: FormEvent<HTMLFormElement>) {
    ...
    const r = await fetch('/api/admin/licenses', { method:'POST', headers:{'content-type':'application/json'},
      body: JSON.stringify({ customerNote: fd.get('note'), maxDevices: Number(fd.get('maxDevices')),
        expiresAt: fd.get('expiresAt') || null }) });
    const j = await r.json();
    if (!r.ok) { setMessage('创建失败：' + (j.error ?? 'unknown')); return; }
    setMessage('新授权码（仅此处完整显示，请立即保存）：' + j.licenseCode);
    e.currentTarget.reset(); await load();
  }
  ...
  async function action(id: string, body: object) {
    ...
    if (!r.ok) { setMessage('操作失败'); return; }
    setMessage('操作成功'); await load();
  }

  async function logout() {
    await fetch('/api/admin/logout', { method:'POST' }); setLicenses([]); setLoggedIn(false); setMessage('');
  }
  ...
  return <div className="adminPanel">
    ...
    <form className="adminForm" onSubmit={createLicense}>
      <h3>创建授权码</h3>
    ...
    <div className="licenseList">{licenses.length === 0 ? <p className="muted">暂无授权。</p> : licenses.map(x =>
      <article className="licenseRow" key={x.id}>
        <div><strong>{x.license_prefix}••••</strong><div className="muted">{x.customer_note || '无备注'}</div></div>
