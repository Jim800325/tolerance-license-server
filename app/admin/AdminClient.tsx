...
export default function AdminClient() {
  ...
  const load=useCallback(async()=>{
    try {
      ...
      const r=await fetch('/api/admin/licenses',{cache:'no-store',signal:controller.signal});
      clearTimeout(timer);
      if(r.status===401){setLoggedIn(false);return;}
      if(!r.ok){setLoggedIn(false);setMessage('授权接口异常：HTTP '+r.status+'。请刷新页面后重试。');return;}
      const j=await r.json(); setLicenses(j.licenses??[]); setLoggedIn(true);
    } catch {
      setLoggedIn(false); setMessage('无法连接授权接口。请刷新页面；如果持续出现，请检查最新 Vercel 部署日志。');
    }
  },[]);
  ...
  async function remove(id:string,code:string|null){if(!window.confirm('确定永久删除授权 '+(code??'')+'？相关设备记录也会删除，此操作不可恢复。'))return;const r=await fetch('/api/admin/licenses/'+id,{method:'DELETE'});if(!r.ok){setMessage('删除失败');return;}setMessage('授权已永久删除');await load();}
  async function logout(){await fetch('/api/admin/logout',{method:'POST'});setLicenses([]);setLoggedIn(false);setMessage('');}

  if(loggedIn===null)return <p>正在检查管理员会话…（最长约 10 秒）</p>;
  ...
  return <div className="adminPanel"><div className="adminTop"><h2>授权管理</h2><button className="secondary" onClick={logout}>退出</button></div>
    <form className="adminForm" onSubmit={createLicense}><h3>创建授权码</h3><label>客户备注<input name="note" maxLength={500} placeholder="例如：客户 A / 订单号"/></label><label>设备上限<input name="maxDevices" type="number" min="1" max="100" defaultValue="1" required/></label><label>到期时间（可留空永久）<input name="expiresAt" type="datetime-local"/></label><label className="checkboxLabel"><input name="completeScript" type="checkbox" defaultChecked/><span>允许生成完整执行脚本</span></label><button type="submit">生成授权码</button></form>
    {message&&<p className="notice">{message}</p>}<div className="licenseList">{licenses.length===0?<p className="muted">暂无授权。</p>:licenses.map(x=><article className="licenseRow" key={x.id}><div><strong>{x.license_code??x.license_prefix+'••••（旧授权不可恢复）'}</strong><div className="muted">{x.customer_note||'无备注'}</div></div><div>状态：{x.status}<br/>设备：{x.active_devices}/{x.max_devices}<br/>完整脚本：{x.features?.completeScript===false?'关闭':'开启'}<br/>到期：{x.expires_at?new Date(x.expires_at).toLocaleString():'永久'}</div><div className="rowActions"><input className="expiryInput" type="datetime-local" aria-label="修改到期时间" defaultValue={x.expires_at?new Date(new Date(x.expires_at).getTime()-new Date(x.expires_at).getTimezoneOffset()*60000).toISOString().slice(0,16):''} onChange={e=>{const value=e.currentTarget.value;void action(x.id,{expiresAt:value?new Date(value).toISOString():null});}}/><button className="secondary" onClick={()=>action(x.id,{expiresAt:null})}>设为永久</button><button className="secondary" onClick={()=>action(x.id,{features:{...(x.features??{}),completeScript:x.features?.completeScript===false}})}>{x.features?.completeScript===false?'开启完整脚本':'关闭完整脚本'}</button>{x.status!=='active'&&<button onClick={()=>action(x.id,{status:'active'})}>恢复</button>}{x.status==='active'&&<button onClick={()=>action(x.id,{status:'paused'})}>暂停</button>}{x.status!=='revoked'&&<button className="danger" onClick={()=>action(x.id,{status:'revoked'})}>撤销</button>}<button className="secondary" onClick={()=>action(x.id,{action:'reset_devices'})}>重置设备</button><button className="danger" onClick={()=>remove(x.id,x.license_code)}>永久删除</button></div></article>)}</div></div>;
}
